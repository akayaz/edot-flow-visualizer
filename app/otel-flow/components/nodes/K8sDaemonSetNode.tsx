...
import type { K8sDaemonSetNodeData } from '../../types';
import { useNodeColors } from '../../hooks/useNodeColors';

interface K8sDaemonSetNodeProps {
  data: K8sDaemonSetNodeData;
  selected?: boolean;
}

export const K8sDaemonSetNode = memo(({ data, selected }: K8sDaemonSetNodeProps) => {
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2.5 mb-3">
        ...
        <div className="px-2 py-1 rounded bg-cyan-500/20 text-[13px] font-medium" style={{ color: nodeColors.accentSecondary }}>
          For EDOT Agents
        </div>
      </div>
      ...
      {data.nodeSelector && Object.keys(data.nodeSelector).length > 0 && (
        <div className="mb-3">
          <div className="text-sm text-gray-600 dark:text-gray-500 mb-1">Node Selector:</div>
          <div className="flex flex-wrap gap-1">
            {Object.entries(data.nodeSelector).map(([key, value]) => (
              <div
                key={key}
      ...
      <div className="absolute inset-0 flex items-center justify-center pointer-events-none mt-20">
        <div className="text-base text-gray-500 dark:text-gray-600 text-center">
          Drop Collector (Agent) nodes here
        </div>
      </div>
